...
def remove_near_duplicates(df, key_columns, keep_strategy='most_complete'):
    ...
    if keep_strategy == 'most_complete':
        ...
        # We don't reset_index(drop=True) so we can preserve original indices
        df_dedup = df.groupby(key_columns, as_index=False, group_keys=False).apply(keep_most_complete)
