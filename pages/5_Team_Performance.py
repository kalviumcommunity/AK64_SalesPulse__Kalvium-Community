...
with col_table:
    with st.container(border=True):
        ...
        for i, row in df_reps.reset_index(drop=True).iterrows():
            ...
            st.markdown(f"""
            <tr {highlight}>
                <td><span style="font-size:14px;">{rank_icon}</span></td>
                ...
                <td style="min-width:110px;">{att_bar_html}</td>
...
with col_detail:
    ...
    with st.container(border=True):
        ...
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px;">
            ...
            <div>
                <div style="font-size:15px;font-weight:700;color:#0F172A;">{rep_info['name']}</div>
                <div style="font-size:11px;color:#64748B;">{rep_info['role']}</div>
            </div>
        </div>
        <hr class="sp-hr">
        """, unsafe_allow_html=True)
        ...
        st.markdown('<div style="font-size:12.5px;font-weight:700;color:#1E293B;margin-bottom:8px;">🤖 AI Coaching Recommendations</div>', unsafe_allow_html=True)
        ...
        st.markdown('<hr class="sp-hr">', unsafe_allow_html=True)
        st.markdown(f'<div style="font-size:11.5px;color:#64748B;margin-bottom:8px;"><strong>Manager Action:</strong> {rep_info.get("recommended_action","—")}</div>', unsafe_allow_html=True)
