...
Delivers generated structured reports to stakeholder inboxes via SMTP.
Security & Resilience Rules:
  - Credentials read from environment variables (os.environ.get) -- NEVER hardcoded
  - Non-blocking try/except block catches errors without crashing calling app/pipeline
"""
...
# Alias function signature for compatibility
def send_report(report_text, recipient):
    return send_report_email(report_text, recipient)


if __name__ == "__main__":
    test_report = "WEEKLY ANALYTICS REPORT\n== KPI SUMMARY ==\nTotal Revenue: $50,000"
    success = send_report(test_report, "test@example.com")
    print(f"Send attempt result (expected False without env vars): {success}")
